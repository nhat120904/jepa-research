...
#SBATCH --array=0-7%4
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/acm_bridge_%A_%a.out
set -euo pipefail
...
for SOURCE in dataset cem_fixed; do
  "$PY" "$PROJECT/scripts/measure_curvature.py" \
    ...
    --action-source "$SOURCE" --horizons 1,5 \
    --sigmas 0.00125,0.0025,0.005,0.01,0.025,0.05,0.10,0.20 \
    --n-directions 2 --repeats 2 --model-dtype float64 \
    --probe "$PROJECT/outputs/probe_bridge/probe.npz" \
    --out-dir "$PROJECT/outputs/bridge/$SOURCE/snapshot_$(printf '%03d' "$TASK_ID")"
done
