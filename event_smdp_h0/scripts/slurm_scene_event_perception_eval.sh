#!/usr/bin/env bash
#SBATCH --job-name=scene_event_perception_eval
...
#SBATCH --array=0-15%8
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/scene_event_perception_eval_%A_%a.out
set -euo pipefail
...
OUT="$PROJECT/outputs/scene_event_perception/eval/seed${MODEL_SEED}/${INDEX}"
...
sha256sum "$PROJECT/scene_core.py" "$PROJECT/scene_abstract_smdp.py" \
  "$PROJECT/scene_event_perception.py" "$PROJECT/scripts/eval_scene_event_perception.py" \
  "$PROJECT/scripts/slurm_scene_event_perception_eval.sh"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader
"$RUNTIME/.venv/bin/python" "$PROJECT/scripts/eval_scene_event_perception.py" \
  ...
  --views latent,privileged --budgets 14,28,56,112 --horizon 4 --out-dir "$OUT"
