...
#SBATCH --array=0-7%4
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/acm_ord_%A_%a.out
set -euo pipefail
...
for SOURCE in dataset cem_fixed; do
  "$PY" "$PROJECT/scripts/measure_curvature.py" \
    ...
    --out-dir "$PROJECT/outputs/ordinal/$SOURCE/snapshot_$(printf '%03d' "$TASK_ID")"
done
