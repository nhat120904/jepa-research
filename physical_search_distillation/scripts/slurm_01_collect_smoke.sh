#!/usr/bin/env bash
#SBATCH --job-name=perd_collect_smoke
#SBATCH --partition=main
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=00:45:00
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/perd_collect_smoke_%j.out
set -euo pipefail
...
sha256sum "$PROJECT/core.py" "$PROJECT/scripts/collect_populations.py" "$PROJECT/scripts/slurm_01_collect_smoke.sh"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader
"$STAGE0_ROOT/.venv/bin/python" "$PROJECT/scripts/collect_populations.py" \
  --snapshot-index 1 --manifest "$PROJECT/outputs/h0/manifest.json" \
  --out-dir "$PROJECT/outputs/h0/populations_smoke"
