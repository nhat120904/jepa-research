#!/usr/bin/env bash
#SBATCH --job-name=ti_d_smoke
#SBATCH --partition=mig
#SBATCH --gres=gpu:nvidia_h100_80gb_hbm3_3g.40gb:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=00:20:00
#SBATCH --output=/mnt/data/nhatnc129/jepa/trajectory_innovation/logs/d_smoke_%j.out
# Gate D collection smoke: 2 unused roots (39992-39993).
set -euo pipefail
...
RUN="$ROOT/d_smoke_${SLURM_JOB_ID}"
CODE="$RUN/code"
[[ ! -e "$CODE" ]] || exit 2
mkdir -p "$CODE"
...
"$PY" "$CODE/scripts/d_collect.py" --out "$RUN" --prep "$PREP" --smoke "$SMOKE" --first 39992 --count 2
