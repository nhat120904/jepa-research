#!/usr/bin/env bash
#SBATCH --job-name=scene_h1_train
...
#SBATCH --array=0-1
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/scene_h1_train_%A_%a.out
set -euo pipefail
...
OUT="$PROJECT/outputs/scene_h1/checkpoints/${VIEW}/seed${MODEL_SEED}"
...
sha256sum "$PROJECT/scene_learning.py" "$PROJECT/scripts/train_scene_h1.py" \
  "$PROJECT/scripts/slurm_scene_h1_train.sh"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader
"$RUNTIME/.venv/bin/python" "$PROJECT/scripts/train_scene_h1.py" \
