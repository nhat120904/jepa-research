#!/usr/bin/env bash
#SBATCH --job-name=lscope_restore_cpu
#SBATCH --partition=main
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=00:20:00
#SBATCH --output=/mnt/data/nhatnc129/jepa/latent_scope_baseline/logs/restore_diagnosis_%j.out
set -euo pipefail
...
env MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa \
  ...
  "$RUNTIME_ROOT/sim_venv/bin/python" "$PROJECT_ROOT/latent_scope_20260909/scripts/diagnose_qualification_restore.py" \
  ...
  --output "$RUNTIME_ROOT/outputs/restore_diagnosis_$SLURM_JOB_ID.json"
