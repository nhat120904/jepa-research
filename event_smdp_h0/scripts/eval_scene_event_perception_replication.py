#!/usr/bin/env python3
"""Fresh-reset replication of learned current-q event planning."""
...
def main() -> None:
    ...
    if "SLURM_JOB_ID" not in os.environ:
        raise RuntimeError("replication eval must run inside a Slurm compute job")
    if not torch.cuda.is_available():
        raise RuntimeError("replication eval requires a GPU allocation")
    ...
    for model_seed in observer_seeds:
        path = args.observer_root / "latent" / f"seed{model_seed}" / "observer.pt"
        observer = EventObserverEvaluator(path, device="cuda")
        if observer.feature_view != "latent":
            raise ValueError(f"observer seed {model_seed} is not a latent observer")
        observers[model_seed] = observer
        observer_meta[str(model_seed)] = {
            "path": str(path),
    ...
    try:
        ...
        arm_specs.extend(
            ("learned_latent", observer_seed, ARM_EVENT)
        ...
        for arm_name, observer_seed, feedback_arm in arm_specs:
            ...
            for decision in range(max_decisions):
                if observer_seed is None:
                    ...
                    planning_state, observer_details = observers[observer_seed].predict(
                ...
                replans.append(
                    {
                        ...
                        "stable_probability": (
                            observer_details["stable_probability"]
        ...
        output = {
            ...
            "results": results,
            "scope": (
                "fresh held-out reset; oracle and terminal use simulator current q, while "
                "learned arms infer current q from a newly rendered observation"
            ),
        }
