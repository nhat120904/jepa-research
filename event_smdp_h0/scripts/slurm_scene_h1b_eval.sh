#!/usr/bin/env bash
#SBATCH --job-name=scene_h1b_eval
...
#SBATCH --array=0-15%8
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/scene_h1b_eval_%A_%a.out
set -euo pipefail
...
OUT="$PROJECT/outputs/scene_h1b/eval/seed${MODEL_SEED}/${INDEX}"
...
sha256sum "$PROJECT/scene_core.py" "$PROJECT/scene_abstract_smdp.py" \
  "$PROJECT/scripts/eval_scene_h1b.py" "$PROJECT/scripts/slurm_scene_h1b_eval.sh"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader
"$RUNTIME/.venv/bin/python" "$PROJECT/scripts/eval_scene_h1b.py" \
  --task-id "$TASK" --reset-seed "$RESET_SEED" --model-seed "$MODEL_SEED" \
  --checkpoint "$CHECKPOINT" --budgets 14,28 --horizon 4 --out-dir "$OUT"
