#!/usr/bin/env bash
#SBATCH --job-name=scene_state_vs_feedback
#SBATCH --partition=mig
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=08:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/scene_state_vs_feedback_%j.out
set -euo pipefail
...
sha256sum "$PROJECT/scene_core.py" "$PROJECT/scene_feedback.py" \
  "$PROJECT/scene_event_history.py" \
  "$PROJECT/scripts/eval_scene_state_vs_feedback.py" \
  "$PROJECT/scripts/slurm_scene_state_vs_feedback_chunk.sh"
...
for (( INDEX=START_INDEX; INDEX<=END_INDEX; INDEX++ )); do
  ...
  OUT="$PROJECT/outputs/scene_state_vs_feedback/eval/${RUN_ID}/${INDEX}"
  echo "INDEX=$INDEX TASK=$TASK SEED=$RESET_SEED START=$(date -u +%FT%TZ)"
  "$RUNTIME/.venv/bin/python" "$PROJECT/scripts/eval_scene_state_vs_feedback.py" \
