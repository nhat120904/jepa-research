#!/usr/bin/env bash
#SBATCH --job-name=spwm_eval
#SBATCH --partition=mig
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --time=12:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/spwm_eval_%j.out
set -euo pipefail
...
sha256sum "$PROJECT/scene_eval.py" "$PROJECT/scene_render.py" \
          "$PROJECT/progress_head.py" "$PROJECT/progress_objective.py" \
          "$PROJECT/scripts/eval_scene_plan.py" \
          "$PROJECT/scripts/slurm_eval_ladder.sh"
...
for OFFSET in $OFFSETS; do
  ...
  "$PY" "$PROJECT/scripts/eval_scene_plan.py" \
    --checkpoint "$CKPT" \
    --cache-dir "$CACHE_ROOT/cache/val" \
    --out-dir "$PROJECT/outputs/ladder/eval/$RUN_ID" \
    "${OBJECTIVE_ARGS[@]}" \
    --goal-offset "$OFFSET" \
