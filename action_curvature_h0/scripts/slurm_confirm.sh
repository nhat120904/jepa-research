...
#SBATCH --array=8-63%2
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/acm_conf_%A_%a.out
set -euo pipefail
...
for SOURCE in dataset cem_fixed; do
  "$PY" "$PROJECT/scripts/measure_curvature.py" \
    ...
    --out-dir "$PROJECT/outputs/confirm/$SOURCE/snapshot_$(printf '%03d' "$TASK_ID")"
done
