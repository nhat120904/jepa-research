...
| 5.11 | The deployed stateprobe **scalar cost** $\hat c$ has *not* been evaluated end-to-end on the held-out expert split; only its two decoded channels have | absence of artifact; `results/stateprobe_cem_validation.md` covers channels only | ⛔ open validation item — paper must not imply the composite cost was validated (§4.2 and Limitations now state this explicitly) |
