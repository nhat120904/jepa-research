#!/usr/bin/env bash
#SBATCH --job-name=scene_abstract_terminal
...
#SBATCH --array=0-15%8
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/scene_abstract_terminal_%A_%a.out
set -euo pipefail
...
OUT="$PROJECT/outputs/scene_abstract_factorial/terminal/seed${MODEL_SEED}/${INDEX}"
...
sha256sum "$PROJECT/scene_core.py" "$PROJECT/scene_abstract_smdp.py" \
  "$PROJECT/scripts/eval_scene_abstract_terminal.py" \
  "$PROJECT/scripts/slurm_scene_abstract_terminal.sh"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader
"$RUNTIME/.venv/bin/python" "$PROJECT/scripts/eval_scene_abstract_terminal.py" \
