#!/usr/bin/env bash
#SBATCH --job-name=scene_event_perception_repl_chunk
#SBATCH --partition=main
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=02:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/scene_event_perception_repl_chunk_%j.out
set -euo pipefail
...
sha256sum "$PROJECT/scene_core.py" "$PROJECT/scene_abstract_smdp.py" \
  ...
  "$PROJECT/scripts/slurm_scene_event_perception_replication_chunk.sh"
