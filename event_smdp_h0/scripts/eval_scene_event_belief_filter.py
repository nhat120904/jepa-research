#!/usr/bin/env python3
"""Evaluate a prediction--correction event-state filter on fresh Scene resets."""
...
def join_observation_with_prior(
    observation: MilestoneState, prior: MilestoneState
) -> MilestoneState:
    """Product-lattice correction; current stability is deliberately not latched."""
    return replace(
        observation,
        cube_stage=max(observation.cube_stage, prior.cube_stage),
        window_stage=max(observation.window_stage, prior.window_stage),
    )
...
def main() -> None:
    ...
    if "SLURM_JOB_ID" not in os.environ:
        raise RuntimeError("belief-filter eval must run inside a Slurm compute job")
    if not torch.cuda.is_available():
        raise RuntimeError("belief-filter eval requires a GPU allocation")
    ...
    observers: dict[int, EventObserverEvaluator] = {}
    observer_meta: dict[str, dict[str, str]] = {}
    for observer_seed in observer_seeds:
        path = args.observer_root / "latent" / f"seed{observer_seed}" / "observer.pt"
        observers[observer_seed] = EventObserverEvaluator(path, device="cuda")
        observer_meta[str(observer_seed)] = {
            "path": str(path),
    ...
    try:
        ...
        for observer_seed in observer_seeds:
            arms.append(("fresh_q", observer_seed, ARM_EVENT))
            arms.append(("filtered_q", observer_seed, ARM_EVENT))

        for arm_name, observer_seed, feedback_arm in arms:
            snapshots.restore(root)
            true_state = initial_milestones(args.task_id)
            filter_prior = initial_milestones(args.task_id)
            deployed: list[str] = []
            ...
            for decision in range(max_decisions):
                if observer_seed is None:
                    ...
                    observation_state, details = observers[observer_seed].predict(
                        feature, goal, args.task_id
                    )
                    stable_probability = float(details["stable_probability"])
                    planning_state = (
                        join_observation_with_prior(observation_state, filter_prior)
                        if arm_name == "filtered_q"
                        else observation_state
                    )
                observation_correct = (
                    observation_state.cube_stage == true_state.cube_stage
                    and observation_state.window_stage == true_state.window_stage
                    and observation_state.stable_success == true_state.stable_success
                )
                planning_correct = (
                    planning_state.cube_stage == true_state.cube_stage
                    and planning_state.window_stage == true_state.window_stage
                    and planning_state.stable_success == true_state.stable_success
                )
                search_seed = (
                    args.seed
                ...
                prior_before = filter_prior
                true_state, record = library.execute(search.selected_action, true_state)
                deployed.append(SKILLS[search.selected_action])
                if arm_name == "filtered_q":
                    predicted = transition.rollout_details(
                        args.task_id, planning_state, (search.selected_action,)
                    )["state"]
                    if not isinstance(predicted, MilestoneState):
                        raise TypeError("transition did not return a MilestoneState")
                    filter_prior = replace(predicted, stable_count=0)
                replans.append(
                    {
                        "decision": decision,
                        "true_state": asdict(state_before),
                        "observation_state": asdict(observation_state),
                        "planning_state": asdict(planning_state),
                        "filter_prior_before": asdict(prior_before),
                        "observation_q_correct": (
                            observation_correct if observer_seed is not None else None
                        ),
                        "planning_q_correct": (
                            planning_correct if observer_seed is not None else None
                        ),
        ...
        output = {
            ...
            "results": results,
            "scope": (
                "fresh held-out belief-filter pilot; filter receives learned transition "
                "prior and current latent observation but never simulator q"
            ),
        }
