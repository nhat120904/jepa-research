#!/usr/bin/env python3
"""Run learned Scene evaluators in the locked receding-horizon Skill-UCT loop."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--feature-views", default=",".join(FEATURE_VIEWS))
...
def main() -> None:
    ...
    if "SLURM_JOB_ID" not in os.environ:
        raise RuntimeError("learned evaluation must run inside a Slurm compute job")
    if not torch.cuda.is_available():
        raise RuntimeError("learned evaluation requires a GPU allocation")

    views = tuple(value for value in args.feature_views.split(",") if value)
    heads = tuple(value for value in args.heads.split(",") if value)
    budgets = tuple(sorted({int(value) for value in args.budgets.split(",") if value}))
    if not set(views) <= set(FEATURE_VIEWS):
        raise ValueError(f"unsupported feature views: {views}")
    if not set(heads) <= set(HEADS):
    ...
    evaluators: dict[tuple[str, str], LearnedSceneEvaluator] = {}
    checkpoint_meta: dict[str, dict[str, str]] = {}
    for view in views:
        for head in heads:
            path = args.checkpoint_root / view / f"seed{args.model_seed}" / f"{head}.pt"
            evaluators[(view, head)] = LearnedSceneEvaluator(path, device="cuda")
            checkpoint_meta[f"{view}/{head}"] = {
                "path": str(path),
    ...
    try:
        ...
        for view in views:
            for head_name in heads:
                evaluator = evaluators[(view, head_name)]
                for budget in budgets:
                    ...
                    for decision in range(max_decisions):
                        ...
                        # Deliberately independent of budget: at a fixed physical
                        # root the K-searches are prefixes of the same UCT run.
                        search_seed = (
                            args.seed
        ...
        output = {
            ...
            "interpretation": (
                "learned evaluator with exact physical deployment; current automaton "
                "state is monitored from the simulator at each real replan"
            ),
            "task_id": args.task_id,
            "reset_seed": args.reset_seed,
            "model_seed": args.model_seed,
