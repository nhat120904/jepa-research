#!/usr/bin/env bash
#SBATCH --job-name=lscope_b0_profile
#SBATCH --partition=main
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=64G
#SBATCH --time=04:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa/latent_scope_baseline/logs/stage_b_profile_%j.out
...
OUTPUT_DIR="$RUNTIME_ROOT/outputs/stage_b_profile_$SLURM_JOB_ID"
PORT=$((20000 + SLURM_JOB_ID % 20000))

test -f "$RUNTIME_ROOT/outputs/setup_result.json"
mkdir -p "$OUTPUT_DIR"
...
EXTRA_ARGS=()
if [[ -n "${STAGE_B_CANDIDATE_COUNT:-}" ]]; then
    EXTRA_ARGS+=(--candidate-count "$STAGE_B_CANDIDATE_COUNT")
fi
if [[ -n "${STAGE_B_PREFIXES_PER_TASK:-}" ]]; then
    EXTRA_ARGS+=(--prefixes-per-task "$STAGE_B_PREFIXES_PER_TASK")
fi

env PYTHONPATH="$GROOT_SRC" \
    "$POLICY_VENV/bin/python" \
    "$PROJECT_ROOT/latent_scope_20260909/scripts/run_groot_policy_server.py" \
    --config "$PROJECT_ROOT/latent_scope_20260909/configs/stage_b_profile.json" \
...
env MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa \
    ...
    "$SIM_VENV/bin/python" \
    "$PROJECT_ROOT/latent_scope_20260909/scripts/run_stage_b_profile.py" \
    --config "$PROJECT_ROOT/latent_scope_20260909/configs/stage_b_profile.json" \
