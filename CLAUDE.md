...
- **DROID (secondary): BOTH baselines complete (2026-06-22, H100).** `dino_wm_droid` + `vjepa2_ac_droid` extracted + scored on the same 333-ep/2331-transition subset (`results/droid_diagnostic.csv`, 32 rows; full writeup `results/vjepa2_ac_droid_completion.md`). hard_nn/hard_effect at the 16-way chance floor in pre-grasp/gripper/contact for **both**; V-JEPA-2-AC (ViT-G/16, 1.01B) is if anything *more* action-blind (AUG≈0 everywhere) than DINO-WM (ViT-S, 22M) — **scaling the encoder 45× does not fix action-grounding.** Boundary Blindness reproduces: pre_grasp BB_boundary +1.94 ≫ free_space +0.68 (`results/droid_boundary.csv`). Gripper sanity gate PASS both; the vjepa2 null is real (fact_to_next 533.9 ≈ zero_to_next 525.3 — the true action gets the prediction no closer to truth than a zero action), not a plumbing artifact. Ran on the SLURM H100 cluster; the old 24 GB-VRAM gate is gone. (`jepa_wm_droid` still out — gated DINOv3 weights, a licensing not hardware blocker.)
