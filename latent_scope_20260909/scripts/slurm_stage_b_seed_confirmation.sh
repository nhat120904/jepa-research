#!/usr/bin/env bash
#SBATCH --job-name=lscope_seed_confirm
...
mkdir "$OUTPUT_DIR"
# Reuse the exact simulator/policy helpers used in 52419.
cp -a "$RUNTIME_ROOT/outputs/qualification_52419/scripts" "$OUTPUT_DIR/scripts"
cp "$PROJECT_ROOT/latent_scope_20260909/scripts/run_stage_b_seed_confirmation.py" "$OUTPUT_DIR/scripts/"
cp "$RUNTIME_ROOT/outputs/qualification_52419/config.json" "$OUTPUT_DIR/config.json"
...
env MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa \
 ...
 "$RUNTIME_ROOT/sim_venv/bin/python" "$OUTPUT_DIR/scripts/run_stage_b_seed_confirmation.py" \
