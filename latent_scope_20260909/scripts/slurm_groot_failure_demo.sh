#!/usr/bin/env bash
#SBATCH --job-name=lscope_fail_demo
#SBATCH --partition=main
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=64G
#SBATCH --time=03:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa/latent_scope_baseline/logs/fail_demo_%j.out
...
OUTPUT_DIR="$RUNTIME_ROOT/outputs/fail_demo_$SLURM_JOB_ID"
...
env PYTHONPATH="$GROOT_SRC" \
    ...
    --config "$PROJECT_ROOT/latent_scope_20260909/configs/groot_failure_demo.json" \
...
env MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa \
    ...
    "$SIM_VENV/bin/python" \
    "$PROJECT_ROOT/latent_scope_20260909/scripts/run_groot_failure_demo.py" \
    --config "$PROJECT_ROOT/latent_scope_20260909/configs/groot_failure_demo.json" \
