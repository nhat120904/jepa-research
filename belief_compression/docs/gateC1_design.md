...
**What Gate C1 has to buy.** Gate A left exactly two defensible novelties: (1) multi-goal invariance of the compression, with a bound holding *jointly* across a goal family; (2) an amortized flip-prediction VOI operating on the compressed M-mode representation. Gate C0 returned **WEAK**: M saturates only along *resolution-refinement* axes, is exactly lossless for the terminal commit but 1.1% lossy closed-loop, the compute ratio flattens in K at fixed horizon (capped by an unavoidable O(K) belief read) and only approaches K/M as H grows, and S6 found compression preserves the *argmax* but not the *value*, which is what probing decisions depend on. Gate C1 must show those properties survive a learned belief over pixels — or kill the program.
