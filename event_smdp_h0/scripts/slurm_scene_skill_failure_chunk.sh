#!/usr/bin/env bash
#SBATCH --job-name=scene_skill_failure
#SBATCH --partition=mig
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=12:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa_runs/logs/scene_skill_failure_%j.out
set -euo pipefail
...
RUN_ID=${RUN_ID:-failure_20260904}
BASE=88500
...
sha256sum "$PROJECT/scene_feedback.py" "$PROJECT/scene_event_history.py" \
  "$PROJECT/scripts/eval_scene_skill_failure.py" \
  "$PROJECT/scripts/slurm_scene_skill_failure_chunk.sh"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader

for (( INDEX=START_INDEX; INDEX<=END_INDEX; INDEX++ )); do
  RESET_SEED=$((BASE + INDEX))
  OUT="$PROJECT/outputs/scene_skill_failure/eval/${RUN_ID}/${INDEX}"
  echo "INDEX=$INDEX SEED=$RESET_SEED START=$(date -u +%FT%TZ)"
  "$RUNTIME/.venv/bin/python" "$PROJECT/scripts/eval_scene_skill_failure.py" \
    --reset-seed "$RESET_SEED" --observer-seeds 0,1,2 \
