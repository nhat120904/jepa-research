...
def main(config_path: str) -> int:
    ...
    max_plan = int(pcfg.get("max_planning_transitions", 200))
    cem_kw = dict(
    ...
    if not df.empty:
        ...
        df["d_ref_p95"] = d_ref
    out_csv = Path(cfg["output"]["csv"]).with_name("droid_planning.csv")
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_csv, index=False)
    np.savez(out_csv.with_name("droid_planning_pertrans.npz"),
             **{k: np.asarray([p[k] for p in pertrans]) for k in
                (pertrans[0].keys() if pertrans else [])})
    print(f"\nWrote {out_csv} ({len(rows)} rows) + per-transition npz ({len(pertrans)} pairs)")
    return 0
