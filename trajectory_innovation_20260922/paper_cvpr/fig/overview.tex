...
\node[ours, anchor=west] (wm) at ({$(a2.east)+(1.3,0)$} |- hist.east) {world model $p_\theta(S\mid C,\bA)$\\\footnotesize JEPA predictor, $M$ code tokens};
...
{$\mathcal{L}_{\mathrm{code}}=\mathcal{L}_{\mathrm{rank}}+\alpha\,\mathcal{L}_{\mathrm{feat}}\;(+\lambda\,\text{predictability})$\\[2pt]
 $\mathcal{L}_{\mathrm{WM}}=-\log p_\theta(\sg(S)\mid C,\bA)$\\[2pt]
 \footnotesize rate fixed by $M\log_2 V$ bits (\cref{sec:problem})};
...
The world model, a joint-embedding predictor, forecasts for each chunk a short discrete code $\hat S$ of the chunk's entire future segment, conditioned on $C$.
A reader that sees $C$ and the code, but never the action, scores each code against goal images.
Since codes do not depend on the goal, one prediction per candidate serves any number of goals.
\emph{Bottom:} during training, a target encoder compresses the actual future segment into the code given $C$.
A reader ranks sibling candidates by their progress labels, and a decoder reconstructs the future's frozen features from $(C,S)$.
The world model learns to predict the stop-gradient code; optionally, code and world model are co-designed for predictability.}
\label{fig:overview}
\end{figure*}
