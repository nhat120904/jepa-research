#!/usr/bin/env bash
#SBATCH --job-name=lscope_a4_collect
#SBATCH --partition=main
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=64G
#SBATCH --time=06:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa/latent_scope_baseline/logs/a4_collect_%j.out
...
OUTPUT_DIR="$RUNTIME_ROOT/outputs/a4_collect_$SLURM_JOB_ID"
...
env MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa \
    ...
    "$SIM_VENV/bin/python" \
    "$PROJECT_ROOT/latent_scope_20260909/scripts/run_stage_a4_collect.py" \
    --config "$PROJECT_ROOT/latent_scope_20260909/configs/stage_a4.json" \
