#!/usr/bin/env bash
#SBATCH --job-name=lscope_base_eval
#SBATCH --partition=main
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=64G
#SBATCH --time=04:00:00
#SBATCH --output=/mnt/data/nhatnc129/jepa/latent_scope_baseline/logs/eval_%j.out
...
OUTPUT_DIR="$RUNTIME_ROOT/outputs/eval_$SLURM_JOB_ID"
...
env MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa \
    PYTHONPATH="$ROBOCASA_SRC:$ROBOSUITE_SRC" \
    "$SIM_VENV/bin/python" \
    "$PROJECT_ROOT/latent_scope_20260909/scripts/run_baseline_sim_client.py" \
    --config "$PROJECT_ROOT/latent_scope_20260909/configs/baseline_policy_gate.json" \
